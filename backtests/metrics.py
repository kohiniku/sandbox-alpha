...
# ---------------------------------------------------------------------------
# Position change helper — shared by cost model and turnover metric
# ---------------------------------------------------------------------------

def _compute_pos_changes(signal, returns_index=None):
    """Absolute position changes, optionally aligned to returns index.

    Used by both apply_trading_cost and calculate_metrics so cost and
    turnover cannot diverge.
    """
    if len(signal) == 0:
        return pd.Series([], dtype=float)
    sig_aligned = signal.shift(1)
    if returns_index is not None:
        sig_aligned = sig_aligned.reindex(returns_index)
    return sig_aligned.diff().abs().fillna(0)
...
def apply_trading_cost(strategy_returns, signal, cost_bps=None):
    """Apply per-trade cost: cost_bps bps per side on each position change.

    cost_bps=None uses the module-level COST_BPS (default 5.0).
    """
    if len(strategy_returns) == 0 or len(signal) == 0:
        return strategy_returns
    effective_cost = COST_BPS if cost_bps is None else cost_bps
    cost_frac = effective_cost / 10000.0  # bps -> decimal
    # Position changes (entry + exit = 2 sides per change)
    pos_changes = _compute_pos_changes(signal, returns_index=strategy_returns.index)
...
def calculate_metrics(returns, signal=None, cost_bps=None):
    """
    ...
    - turnover: annualized one-way turnover = sum(|position changes|) / n_days * 252
    """
    if len(returns) == 0:
    ...
    # Turnover: annualized one-way, same pos_changes as the cost model
    effective_cost = COST_BPS if cost_bps is None else cost_bps
    if signal is not None and len(signal) > 0 and len(returns) > 0:
        pos_changes = _compute_pos_changes(signal, returns_index=returns.index)
        turnover = float(pos_changes.sum()) / len(returns) * 252
    else:
        turnover = 0.0
...
def compute_split_metrics(returns, signal, num_days, cost_bps=None):
    """Build per-split metrics dict including num_days."""
    metrics = calculate_metrics(returns, signal, cost_bps=cost_bps)
